...
export function ModifyBan({ props }) {
    ...
    const [newBanDuration, setNewBanDuration] = useState(user.ban.until ? user.ban.until.toDate() : null);
    ...
    return (
        <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild>
                <Button type="button" className={`w-36 text-lg font-semibold tracking-wide bg-inherit border border-secondary shadow-lg text-secondary hover:text-primary-foreground`}>
                    Modify Ban
                </Button>
            </DialogTrigger>
            <DialogContent className="sm:max-w-[600px] lg:max-w-[720px]">
                ...
                <form onSubmit={handleModifyBan}>
                    <div className="flex flex-col w-full mb-4">
                        <div className="flex flex-col w-full px-10">
                            <div className="flex flex-row items-center justify-start w-full">
                                <Label htmlFor="post-content" className="text-lg w-1/4">Ban Type:</Label>
                                <Select required onValueChange={(value) => setNewBanStatus(value)} value={newBanStatus}>
                            ...
                            <div className="mt-4 flex flex-col w-full">
                                <Label htmlFor="ban-until" className="text-lg mb-2">Ban Until: {newBanStatus === "temporary" && newBanDuration !== null ? handleDateFormat(newBanDuration): "N/A"}</Label>
                                {newBanStatus === "temporary" && 
                                    <Popover>
                                        <PopoverTrigger asChild>
                                        <Button
                                            variant={"outline"}
                                            className={`w-[280px] justify-start text-left font-normal ${!newBanDuration && "text-muted-foreground"}`}
                                        >
                                            <CalendarIcon className="mr-2 h-4 w-4" />
                                            {newBanDuration ? handleDateFormat(newBanDuration) : <span>Pick a date</span>}
                                        </Button>
                                        </PopoverTrigger>
                    ...
                    <DialogClose className="items-center border border-secondary mt-6 p-2 rounded-lg hover:bg-muted hover:scale-110 transition-all duration-300">
                        ...
                        :   <Button type="submit" className="mt-6 hover:scale-110 transition-all duration-300">Modify Ban</Button>}
                    </DialogFooter>
                </form>
            </DialogContent>
        </Dialog>
