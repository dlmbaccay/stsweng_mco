		...
		try {
			...
			await Promise.all(
				reportedPosts.map(async (post) => {
					await updateDocument('posts', post.postID, {
				...
				<div className="mt-2 md:mt-3 flex flex-col">
					<div className="flex flex-row justify-between ">
							...
							<div className="ml-4 items-center justify-center">
								...
								<div id="ban-date" className="flex flex-row gap-2 items-center">
									<p className="text-xs md:text-sm">
										Banned{' '}
										{user.ban.when
											? handleDateFormat(user.ban.when.toDate())
											: 'INDEFINITE'}
									</p>
						...
						<div className="flex flex-col items-end">
							...
							<span className="font-semibold">
								{user.ban.status === 'temporary'
									? handleDateFormat(user.ban.until.toDate())
									: 'INDEFINITE'}
							</span>
						</div>
					</div>
					...
					<div className="mt-3">
						<p className="whitespace-pre-line line-clamp-1 text-sm md:text-base md:line-clamp-4 overflow-hidden text-justify">
							User is{' '}
							{user.ban.status === 'temporary' ? 'TEMPORARILY' : 'PERMANENTLY'} banned
							due to multiple {user.ban.status === 'permanent' && 'verified'} reports.
						</p>
					</div>
				</div>
						...
						<div className="flex flex-row justify-between items-center w-full my-3 px-4">
							...
							>
								...
								<span className="ml-4 text-md font-semibold">
									<span className="text-red-500">{reportedPosts.length}</span>{' '}
					...
					<div className={`${showDetails ? 'flex flex-col mt-6 mx-6' : 'hidden'}`}>
						{reportedPosts &&
							reportedPosts.map((post, index) => (
								<div
									key={index}
									className="w-full mb-4 border border-secondary rounded-lg"
								>
									{' '}
