							...
							<div id="post-meta" className="ml-4 items-center justify-center">
								...
								>
									...
									<Link
										...
										id="username"
										className="hover:text-muted_blue dark:hover:text-light_yellow font-semibold transition-all"
									>
								...
								<div id="publish-date" className="flex flex-row gap-2 items-center">
									...
									{isEdited ? (
										<div className="relative flex flex-row items-center gap-2">
											...
											<p className="edited-post-tooltip hidden text-xs">
												Edited Post
											</p>
										</div>
									) : null}
						...
						{post.taggedPets && post.taggedPets.length > 0 && (
							<div className="flex flex-row items-center justify-center gap-2">
								{post.taggedPets.length === 1 && (
									<i className="fa-solid fa-tag text-xs md:text-md"></i>
								)}
								{post.taggedPets.length > 1 && (
									<i className="fa-solid fa-tags text-xs md:text-md"></i>
								)}
						...
						>
							<div className="flex flex-row justify-start items-start">
								...
								<div id="post-meta" className="ml-4 items-center justify-center">
									...
									>
										<p className="text-xs md:text-sm">
											{handleDateFormat(post.originalPostDate)}
										</p>
									</div>
								</div>
							</div>
										...
										<>
											...
											<i
												...
												onClick={() => {
													setCurrentImageIndex(
														(prevIndex) =>
															(prevIndex + 1) % post.imageURLs.length,
													)
												}}
											></i>
										</>
				...
				<div id="post-footer" className="mt-4 flex flex-col w-full">
					<div className="w-full border-y border-secondary justify-between">
						...
						<div className="flex flex-row justify-between items-center w-full my-3 px-4">
							<div
								...
								className="flex flex-row items-center w-full hover:text-primary hover:cursor-pointer"
							>
								<FontAwesomeIcon
									icon={faCircleInfo}
									className="text-4xl text-primary"
								></FontAwesomeIcon>
								<span id="report-control" className="ml-4 text-md">
									See List of Report Details:
								</span>
								<span className="ml-4 text-md font-semibold">
									<span className="text-red-500">{post.reports.length}</span>{' '}
									Total User Reports
								</span>
							</div>
							<FontAwesomeIcon
								icon={showDetails ? faAngleDown : faAngleRight}
					...
					{post.reportStatus === 'pending' ? (
						<div className="flex w-full justify-center gap-8 mt-4">
							...
							<Button
								type="button"
								onClick={(e) => handleAction('verified')}
								className={`w-36 text-lg font-semibold tracking-wide shadow-lg hover:w-40 hover:text-xl transition-all duration-150 border border-secondary`}
							>
								Verify
							</Button>
							...
							<Button
								type="button"
								onClick={(e) => handleAction('dismissed')}
						...
						<div className="flex w-full justify-center gap-8 mt-4">
							<Button
								type="button"
								onClick={(e) => handleAction('pending')}
								className={`w-36 text-lg font-semibold tracking-wide bg-inherit border border-secondary shadow-lg text-secondary hover:text-primary-foreground`}
							>
								Undo
							</Button>
							<Button
								...
								className={`w-36 text-lg font-semibold tracking-wide shadow-lg border border-secondary`}
							>
								Delete
							</Button>
						</div>
