    ...
    const handleFollow = async () => {
        ...
        try {
            if (pet.followers.includes(currentUser_uid)) {
                await updateDocument('pets', pet.petID, {followers: pet.followers.filter(uid => uid !== currentUser_uid)});
                await updateDocument('users', currentUser_uid, {following: currentUser_following.filter(uid => uid !== pet.petID)});
                toast.success("Unfollowed " + pet.petName);
            } else {
                await updateDocument('users', currentUser_uid, {following: [...currentUser_following, pet.petID]});
    ...
    return (
        ...
        {loading ? <Loader show={true} /> : 
            <Button onClick={handleFollow} className="px-3 h-[35px] bg-primary text-primary-foreground gap-2 flex items-center justify-center">
                {pet.followers.includes(currentUser_uid) ? (
                    <p>Unfollow</p>
                ) : (
                    <p>Follow</p>
                )}
