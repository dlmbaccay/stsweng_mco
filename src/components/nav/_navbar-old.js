...
export default function ExpandedNavBar({props}) {
    ...
    return (
        <>
            <div ref={navRef} className={`h-screen border-r border-gray flex flex-col justify-between bg-white dark:bg-dark_gray ${isExpanded ? "w-64 transition-all duration-300": "w-20 transition-all duration-300"}`}>
                <>
                {/* user meta */}
                <div className="pt-8 mx-auto">
                    <button 
                        ...
                        className='group flex flex-col justify-center items-center gap-4'>
                            {<Image src={userPhotoURL} alt={'profile picture'} width={`${isExpanded ? 120: 50}`} height={`${isExpanded ? 120: 50}`} className='rounded-full aspect-square object-cover'/>}
                    </button>
                    <div className="mx-auto flex flex-col items-center justify-center gap-4 mt-4">
                        {isExpanded && <span className="text-md tracking-wide font-medium">@{username}</span>}
                ...
                <div className="pb-8 text-sm">
                    <div className={`flex flex-col gap-6 ${isExpanded ? "justify-items-start" : ""}`}>
                        ...
                        {/* Home Button */}
                        <button 
                            onClick={() => router.push('/home')} className={`${isExpanded ? "ml-14" : "mx-auto"} transition-all duration-300 flex flex-row items-center gap-3 ${activeTab === '/home' ? 'bg-gray-200 dark:bg-gray-800' : ''}`}>
                    ...
                    {/* Log Out Button */}
                    <button onClick={() => handleSignOut()} className={`${isExpanded ? "ml-14" : "mx-auto"} transition-all duration-300 flex flex-row items-center gap-3`}>
                        <div className="p-1 bg-muted_blue rounded-full dark:bg-light_yellow">
