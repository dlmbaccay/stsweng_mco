...
import { uploadPostMedias } from '@/lib/storage-funcs'
...
export function CreatePost({ props }) {
	...
	const createPost = async (event) => {
		...
		if (postContent === '' || postContent === null || postContent === undefined) {
		...
		setLoading(true)
		try {
			// Implement the create post logic here no API
			const postID = await firestore.collection('posts').doc().id

			let imageURLs = []

			if (mediaFiles.length > 0) {
				imageURLs = await uploadPostMedias(postID, mediaFiles)
			}
			...
			await createPostDocument('posts', postID, postDetails)
		...
		<Dialog open={open} onOpenChange={setOpen}>
			...
			<div className="flex flex-row items-center">
				...
				<DialogTrigger asChild>
					...
					>
						<FontAwesomeIcon icon={faImage} className="h-5 w-5"></FontAwesomeIcon>
					</Button>
				</DialogTrigger>
			</div>
					...
					<div className="mb-4 flex w-full flex-col">
						<div className="flex w-full flex-row gap-12 px-10">
							...
							<div className="flex w-2/5 flex-col items-center">
								<Label htmlFor="category" className={'my-4 w-full'}>
									{' '}
									Post Category{' '}
								</Label>
								<Select
							...
							<div className="flex w-3/5 flex-col items-center">
								<Label htmlFor="pets" className={'text-secondary my-4 w-full'}>
									Tag your Pets!
								</Label>
								<MultiSelect
									...
									className={'text-secondary w-full rounded-md'}
								/>
							</div>
						</div>
						...
						{(postCategory == 'Lost Pets' ||
							...
							<div className="flex w-full flex-col px-10">
								<Label htmlFor="pet-location" className={'my-4 w-full'}>
									Last Seen at{' '}
								</Label>
								<Input
									type="text"
									...
									onChange={(e) => setPostTrackerLocation(e.target.value)}
								/>
							</div>
						)}
						...
						<div className="flex w-full flex-col px-10">
							...
							<div className="items-left flex w-full flex-col">
								<Label htmlFor="media" className={'my-4 w-full'}>
									Upload Media{' '}
								</Label>
								<Input
									id="media"
