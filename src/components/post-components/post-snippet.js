				...
				<Card className="w-full drop-shadow-md hover:drop-shadow-md min-h-fit rounded-md p-6 flex flex-col">
						...
						<div className="flex flex-row justify-start items-start">
							<div id="author-image">
								<Image
									...
									width={50}
									height={50}
									className="rounded-full drop-shadow-sm aspect-square object-cover h-[40px] w-[40px] md:h-[45px] md:w-[45px] cursor-pointer hover:opacity-60"
									onClick={() => {
					...
					<div id="post-body" className="mt-2 md:mt-3 flex flex-col">
						...
						{post.reportStatus === 'verified' ? (
							<div className="items-center mt-5 w-full flex justify-center">
								<span>
									This post violates our guidelines and has been taken down.
								</span>
							</div>
						) : (
							<>
					...
					>
						...
						>
							...
							>
								<DialogTrigger asChild>
									<i className="fa-solid fa-comment hover:text-muted_blue dark:hover:text-light_yellow hover:cursor-pointer transition-all" />
								</DialogTrigger>
								<p>{commentsLength}</p>
							</div>
									...
									<DialogContent>
										...
										<div className="flex flex-col gap-4 w-full h-full">
											<textarea
												...
												value={repostBody}
												onChange={(event) =>
													setRepostBody(event.target.value)
												}
												cols="30"
												rows="10"
