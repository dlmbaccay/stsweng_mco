...
export function Comment({ props }) {
		...
		try {
			...
			if (response.ok) {
				...
				if (currentUser.uid !== authorID) {
					...
					const notifResponse = await addNotification(authorID, notifData)
					// if (notifResponse) {
	...
	return (
		<div className="flex flex-row w-full items-start mih-h-[60px] max-h-fit gap-2">
			<Image
				src={authorPhotoURL ? authorPhotoURL : '/images/profilePictureHolder.jpg'}
				alt={authorUsername}
				width={40}
				height={40}
				...
				<div className="flex flex-col gap-1 w-full items-start break-all py-2 px-3 text-sm rounded-xl drop-shadow-sm bg-off_white dark:bg-gray">
					...
					{!isEditingComment && (
						<div>
							...
							{allReactions.filter((reaction) => reaction.userIDs.length > 0).length >
								0 && (
					...
					<div className="flex flex-row w-full text-xs gap-2 mt-1">
						...
						<div id="date-control" className="flex gap-2 items-center">
							...
							{isEdited ? <div className="italic text-xs">Edited</div> : null}
						</div>
					</div>
