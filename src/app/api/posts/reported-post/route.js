...
import sendBanEmail from '@/lib/sendEmail'
	...
	try {
		...
		if (post.reports.some((report) => report.reportedBy.uid === reportData.reportedBy.uid)) {
			const existingReport = post.reports.find(
				(report) => report.reportedBy.uid === reportData.reportedBy.uid,
			)
		...
		await updateDocument('posts', post.postID, {
			reports: [...post.reports, reportData],
			reportStatus: post.reportStatus ? post.reportStatus : 'pending',
		})
		...
		const snapshot = await firestore
			...
			.where('reportStatus', 'in', ['pending', 'verified'])
			.get()
		...
		if (totalReports >= 10) {
			...
			try {
				...
				await updateDocument('users', post.authorID, {
					ban: { status: 'temporary', until: banUntil, when: new Date() },
				})
				...
				const adminDocRef = firestore
					.collection('admin')
					.doc('5QMdCpbNvBMBSJ0wY9i28adWdx72')
				await adminDocRef.collection('notifications').add(notifData)

				const user = await firestore.collection('users').doc(post.authorID).get()
				const email = user.data().email

				await sendBanEmail(email, 'temporary')
			} catch (err) {
...
export async function PATCH(request) {
	const body = await request.json()
	const { action, id, post, status } = body
	try {
		if (action == 'update-status') {
			...
			if (await checkIfBannable(post.authorID)) {
				await updateDocument('users', post.authorID, {
					ban: { status: 'permanent', until: 'none' },
				})
				// Create a notification for admin
				const notifData = {
					type: 'ban',
				...
				await sendBanEmail(email, 'permanent')
			}
